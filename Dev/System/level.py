...
from event.event import EventCtrl, ChangeMapParam
...
class Level:
    def __init__(self, save_data, map_name, change_map_param = None):
        ...
        self.eventctrl_sprites = pygame.sprite.Group()
        ...
        # save
        self.save = Save(self.menu, self.player)
        self.is_saving = False

        # event class
        self.eventctrl = EventCtrl(self.player, self.eventctrl_sprites, map_name, change_map_param, self.menu)
            ...
            for row_index,row in enumerate(layout):
                for col_index, col in enumerate(row):
                    if col != '-1':
                        ...
                        if style == 'event':
                            Tile((x,y), [self.eventctrl_sprites], 'event', event_id = int(col))
